...
setupWorkspace(workspaceConfig) {
  ...
  _switchToWorkspace(targetWorkspace, fullRestore := 0) {
    ...
    loop (target.list.Length) {
      ...
      skipShow := 0
      skipMove := !fullRestore
      skipMinMax := !fullRestore
      if (currentI) {
      ...
      if fullRestore && targetWnd.minMax != 0
        skipMove := 1
      if !skipMinMax {
        skipShow := 1
        switch targetWnd.minMax {
          case -1:
            CallAsync(WinMinimize, targetWnd.id)
          case 0:
            CallAsync(WinRestore, targetWnd.id)
          case 1:
            CallAsync(WinMaximize, targetWnd.id)
        }
      }
      if !skipMove
